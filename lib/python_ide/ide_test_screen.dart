...
class _IdeScreenState extends State<IdeScreen> {
  ...
  void initState() {
    ...
    // Add this to verify server is ready (Optional but recommended)
    // checks immediately when screen loads
    _pingServer(); 
  }

  // Add this helper function to check connection silently
  Future<void> _pingServer() async {
      await Future.delayed(const Duration(seconds: 2)); // Give Python time to boot
      try {
        final url = Uri.parse('http://127.0.0.1:5000/execute'); 
        // Just checking connectivity, code doesn't matter
        await http.post(url, body: {'code': 'print("ping")'}).timeout(const Duration(seconds: 2));
        debugPrint("✅ Python Server is Ready!");
      } catch (e) {
        debugPrint("⚠️ Python Server might not be ready yet: $e");
      }
  }
